...
df2SCE <- function(df, markers = NULL, non_markers = NULL, sample_col = 'sample', panel = NULL,
  ...
  # Check for packages
  missing_package("SingleCellExperiment", "Bioc")
  ...
  sapply(markers, function(x) {check_colname(colnames(df), x, "df")})
  ...
  if (!is.null(panel) & is.data.frame(panel)) {
    ...
    sapply(c(panel_channel, panel_antigen, panel_type), function(x) {check_colname(colnames(panel), x, "panel")})
