...
if(FALSE){
  ...
  save(preprocessed, file = "_data/01_dfci1_preprocessed.Rdata")
  som3 <- preprocessed %>%
    scale_expr() %>%
    create_som(seed = 473)
  #
  ...
  corrected <- preprocessed %>%
    batch_correct()
  # Save result
  save(corrected, file = "_data/02_dfci1_corrected.Rdata")
  ...
  plot_density(uncorrected = preprocessed,
                corrected = corrected,
                markers = markers,
                filename = "figs/03_dfci1_densities_withcovar_700k.png")
  ...
  plot_save_two(pca1, pca2, filename = "figs/02_dfci1_pca_700k.png")
  ...
  plot_save_two(umap1, umap2, filename = "figs/02_dfci_umap_700k.png")
  library(patchwork)
  umap1 + umap2
  ...
  # Run clustering
  corrected <- corrected %>%
    dplyr::mutate(label3 = som$unit.classif)
  #run_flowsom(., k = 10))
  preprocessed <- corrected %>%
    dplyr::select(id, label) %>%
    dplyr::right_join(preprocessed, by = "id")
