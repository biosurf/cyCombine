...
The flow cytometry dataset consists of 23 bone marrow aspirates from healthy donors, which were frozen and thawed, stained with a total of 47 different antibodies across 5 panels and analyzed using a 5-laser flow cytometer (BD Biosciences LSRFortessa). Pre-processing was done in FlowJo version 10 (Tree Star Inc). Singlets and non-debris were identified using forward and side-scatter. Dead cells were excluded using live/dead stains.
...
```{r loading flow data 1, message=F, warning=F}
...
```{r cytof read, warning=F, message=F}
# Directory with raw .fcs files
...
```{r density plot, message=FALSE, fig.height=6, fig.width=16}
plot_density(uncorrected, corrected, ncol = 4, xlims = c(-3,10))

```

<br>

And now some UMAPs to visualize the correction. I will downsample to 10,000 cells from each dataset so it is easier to see what is going on.
...
```{r marker umaps 2, fig.height=8, fig.width=10}
...
Here, it is seen that the sample cell type fractions correlate nicely with the counterpart from the other technology. However, we do have some clusters, e.g. cluster 1, where the fractions tend to be larger for one technology.
