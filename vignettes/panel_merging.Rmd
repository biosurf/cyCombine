...
panel_A <- prepare_data(data_dir = data_dir, pattern = '#A', down_sample = F, batch_ids = 'A')
panel_B <- prepare_data(data_dir = data_dir, pattern = '#B', down_sample = F, batch_ids = 'B')
panel_C <- prepare_data(data_dir = data_dir, pattern = '#C', down_sample = F, batch_ids = 'C')
panel_D <- prepare_data(data_dir = data_dir, pattern = '#D', down_sample = F, batch_ids = 'D')
panel_E <- prepare_data(data_dir = data_dir, pattern = '#E', down_sample = F, batch_ids = 'E')
```
...
```{r cycombine evaluation, results='hide', message=F, warning=F, fig.width=20, fig.height=7}
...
boxplot(emd_all, main = 'EMD for all markers (cyCombine)', ylab = 'EMD', col = c('#F8766D', '#619CFF')[as.numeric(names(emd_all) %in% overlap_AB)+1], ylim = c(0,5))
...
```{r cytobackbone evaluation, results='hide', message=F, warning=F, fig.width=20, fig.height=7}
# Combine all datasets
...
boxplot(emd_all_CBB, main = 'EMD for all markers (CytoBackBone)', ylab = 'EMD', col = c('#F8766D', '#619CFF')[as.numeric(names(emd_all_CBB) %in% overlap_AB)+1], ylim = c(0,5))
legend("topright", legend = c('Overlap', 'Imputed'), col = c('#619CFF', '#F8766D'), pch = 15, cex = 2, bty = 'n')
```

Since the CytoBackBone function does not maintain the information about which panel, which cell originally came from after merging, there is no way to obtain all five EMD values. However, we will still compare the EMD boxplots directly. The EMD ranges are similar - i.e. both tools seem to be most challenged with markers such as CXCR4 and MIP1b. 


A final comparison is the runtime. Here we do a small comparison of cyCombine and CytoBackBone on a machine with 40 cores and 100 gb memory:
...
While neither of these runtimes are terrible, it is worth considering that cyCombine is a lot faster. 

<br> 

In conclusion, the two tools have very similar performance in terms of the global distributions as seen from both the EMD calculations and the density plots. However, cyCombine integrates more cells and has a shorter runtime.
