...
vignette: >
  %\VignetteIndexEntry{Integrating spectral flow and CyTOF data}
...
```{r loading spectral flow data 1}
...
I will impute all the markers missing in either dataset first.
...
# Let us make the UMAP for the corrected set (based on overlapping markers only)
umap_plot <- plot_dimred(spectral[sample,], name = 'Corrected', type = 'umap', plot = 'CD19', return_coord = T)
...
# We can further include the UMAP coordinates in a combined dataframe with both imputed datasets. Again, we downsample to avoid too many cells cluttering the view (the same indeces are used as before)
combined <- cbind.data.frame(umap_plot$dimred, spectral[sample,])
